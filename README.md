...
(function () {
  ...
  const state = {
    ...
    review: {
      ...
      reverseMode: false, reversePagesPerDay: 6,
    },
  ...
  function updateMemoResults() {
    ...
    if (m.mode === "duration") {
      ...
      html += inspireHTML(`بإذن الله، بناءً على خطتك ستبدأ بتاريخ ${formatDualDate(getPlanStartDate())} وتختم حفظ ${hasPreMem ? "بقية" : ""} القرآن الكريم بتاريخ ${formatDualDate(r.finishDate)}`);
    } else {
      ...
      html += inspireHTML(`بإذن الله، بناءً على معدلك ستبدأ بتاريخ ${formatDualDate(getPlanStartDate())} وتختم حفظ ${hasPreMem ? "بقية" : ""} القرآن الكريم بتاريخ ${formatDualDate(r.finishDate)}`);
    }
    if (hasPreMem) {
    ...
    const memoDays = buildDailyAssignments(startPage, TOTAL_PAGES, totalDays, getEffectiveActiveDays(m), getPlanStartDate());
    html += buildUpcomingScheduleTableHTML(memoDays, { title: "النطاق اليومي والتواريخ (حفظ)", rangeColLabel: "ورد الحفظ" });
  ...
  function buildReviewFormHTML(r, prefix) {
    ...
    const scheduleFieldsHTML = r.cyclicMode ? `
      ...
      <div class="card form-card">
        ...
        <p class="hint">تبدأ الخطة من الصفحة ${rangeForHints.to} (الأقرب حفظاً) وتتراجع يوماً بعد يوم حتى الوصول إلى الصفحة ${rangeForHints.from}، دون تحديد مدة مسبقة.</p>
        ${reviewDaysSelectHTML}
      </div>` : `
      <div class="card form-card">
  ...
  function updateCombinedResults() {
    ...
    const memoDays = buildDailyAssignments(memoStartPage, TOTAL_PAGES, memoTotalDays, getEffectiveActiveDays(m), planStart);
  ...
  function getActivePlanAssignments() {
    ...
    if (state.tab === "memorize") {
      ...
      const days = buildDailyAssignments(startPage, TOTAL_PAGES, totalDays, getEffectiveActiveDays(m), today);
      return {
        days,
        title: "خطة الحفظ",
        subtitle: hasPreMem
      ...
      if (r.cyclicMode) {
        ...
        const days = buildCyclicAssignments(range.from, range.to, cycleDays, totalDays, getEffectiveActiveDays(r), today);
        const endText = r.endMode === "date"
      ...
      if (r.reverseMode) {
        const perDay = Math.max(Math.round(Number(r.reversePagesPerDay) || 1), 1);
        const days = buildReverseDailyAssignments(range.from, range.to, perDay, getEffectiveActiveDays(r), today);
        return {
          days,
          title: "جدول المراجعة العكسية",
          subtitle: `مراجعة تنازلية من الصفحة ${range.to} إلى الصفحة ${range.from} بمعدل ${perDay} صفحة يومياً`,
        };
      }
      const totalDays = computeTotalDays(r);
      const days = buildDailyAssignments(range.from, range.to, totalDays, getEffectiveActiveDays(r), today);
  ...
  function getCombinedPlanAssignments() {
    ...
    const memoStartPage = getRemainingStartPage(m);
    const memoDays = buildDailyAssignments(memoStartPage, TOTAL_PAGES, memoTotalDays, getEffectiveActiveDays(m), today);
    ...
    if (r.cyclicMode) {
      ...
      const programDays = getProgramDays(r);
      const numCycles = Math.max(Math.round(programDays / cycleDays), 1);
      reviewDays = buildCyclicAssignments(range.from, range.to, cycleDays, programDays, getEffectiveActiveDays(r), today);
      const endText = r.endMode === "date"
      ...
      const perDay = Math.max(Math.round(Number(r.reversePagesPerDay) || 1), 1);
      reviewDays = buildReverseDailyAssignments(range.from, range.to, perDay, getEffectiveActiveDays(r), today);
      reviewSubtitle = `ومراجعة عكسية تنازلية من الصفحة ${range.to} إلى الصفحة ${range.from} بمعدل ${perDay} صفحة يومياً`;
    } else {
      const reviewTotalDays = computeTotalDays(r);
      reviewDays = buildDailyAssignments(range.from, range.to, reviewTotalDays, getEffectiveActiveDays(r), today);
