...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .app-shell {
    ...
    /* "fixed" على الجوال يجبر المتصفح على إعادة رسم الخلفية مع كل إطار تمرير (سبب شائع ومعروف
       لتعليق/تهنيج التمرير على المتصفحات المحمولة)، لذا نجعلها "scroll" افتراضياً (يشمل الجوال)
       ونُفعّل التأثير الخفيف (fixed) فقط على الشاشات الأكبر (حاسوب) عبر media query أدناه. */
  ...
  /* التأثير الخفيف لخلفية ثابتة أثناء التمرير مناسب فقط على الحاسوب حيث لا يوجد تمرير باللمس؛
     على الجوال أعلاه يبقى scroll دائماً لتفادي أي تهنيج في التمرير. */
  @media (min-width: 900px) {
    .app-shell { background-attachment: fixed, scroll; }
  }
  ...
  .cal-cell {
    ...
    flex-direction: column;
    justify-content: space-between;
    overflow: hidden; /* يضمن بقاء أي محتوى طويل (نطاقات صفحات متعددة في الحفظ المعكوس) داخل حدود المربع دوماً */
  }
  ...
  .cal-date-num { font-weight: 700; font-size: 0.85rem; color: var(--coffee); align-self: flex-start; flex: 0 0 auto; }
  .cal-task {
    font-size: 0.72rem;
    font-weight: 700;
    color: var(--espresso);
  ...
  .cal-task.cal-task-long { font-size: 0.6rem; }

  /* ---- Combined (memorize + review) calendar cell ---- */
  .cal-cell.cal-cell-combined { min-height: 0; }
  .cal-task-group { display: flex; flex-direction: column; gap: 4px; min-height: 0; overflow: hidden; }
  .cal-task-mini {
    font-size: 0.62rem;
    font-weight: 700;
    border-radius: 6px;
  ...
  .cal-task-mini.long { font-size: 0.52rem; }
...
(function () {
  ...
  /* ---------------- محاذاة نهاية اليوم على حدود السور (بدل الاعتماد على حساب الصفحات فقط) ----------------
     ...
     وكان ينقصها صفحة واحدة فقط لتختم السورة كاملة، نمدّها بصفحة واحدة لتنطبق نهاية اليوم مع نهاية
     السورة تماماً (فيبدأ اليوم التالي تلقائياً من أول سورة جديدة، تصاعدياً).
     ملاحظة: أُزيل سابقاً تعديل مقابل كان "يسحب" صفحة واحدة من نهاية اليوم إن كانت أول صفحة منفردة من
     سورة جديدة ويؤجلها لليوم التالي — لأن هذا كان يترك أحياناً صفحة يتيمة معزولة (مثل صفحة واحدة من
     سورة فُصّلت) تظهر لوحدها في يوم لاحق بدل أن تُدمج بشكل صحيح مع بقية صفحات نفس السورة، فبدا الأمر
     وكأن هناك صفحات "مفقودة" رغم أنها لم تكن كذلك فعلياً. الآن أي سورة لم تكتمل يبقى الباقي منها
     تلقائياً لليوم التالي، ويبدأ حساب اليوم التالي من حيث توقف (أي من بداية السورة نفسها إن كانت هي
     كل ما تبقى) تصاعدياً دون أي قصّ اصطناعي. */
  ...
  // يرجع نطاقات الصفحات الفعلية ليوم ما: يعتمد أولاً على day.ranges (مصفوفة {from,to} — تُستخدم في
  ...
  function formatDayRangesFull(d) {
    const ranges = dayPageRanges(d);
    return ranges.map(formatPageRangeFull).join("، ثم ");
  }
  // تسمية يوم كامل بالصيغة المختصرة (تُستخدم في التقويم وجدول الخطة الشاملة).
  function formatDayRangesShort(d) {
    const ranges = dayPageRanges(d);
    return ranges.map(formatPageRangeShort).join(" + ");
  }
  ...
  function buildUpcomingScheduleTableHTML(days, opts) {
    ...
    const rangeLabelFn = opts.rangeLabelFn || ((d) => {
      ...
      if (d.isFilled || (d.fromPage == null && !(d.ranges && d.ranges.length))) return "✓ تمّ الختم";
      return formatDayRangesFull(d);
    });
  ...
  function buildCombinedUpcomingScheduleTableHTML(days, opts) {
    ...
    const labelFor = (d) => {
      ...
      if (d.isFilled || (d.fromPage == null && !(d.ranges && d.ranges.length))) return "✓ تمّ";
      return formatDayRangesShort(d);
    };
    const shown = days.slice(0, limit);
    const rows = shown.map((d) => {
      const dayName = WEEK_DAYS[(d.date.getDay() + 1) % 7];
  ...
  /* ---------------- Tabs ---------------- */
  // The PDF button below is shared across all three tabs and always exports
  // whichever plan is on-screen right now (state.tab) — never a plan from a
  // tab you're not currently viewing. Relabeling it here on every tab switch
  // makes that explicit, so it's never ambiguous which plan "حفظ الخطة PDF"
  // is about to save — e.g. designing your plan under "خطة شاملة" but
  // clicking export while "خطة الحفظ" is the active tab used to silently
  // save the memorize-only plan instead, with nothing on screen warning you.
  function updatePdfButtonLabel() {
    const btn = document.getElementById("btn-pdf");
    if (!btn || btn.disabled) return; // don't relabel mid-export ("جارٍ إنشاء PDF...")
    const labels = {
      memorize: "حفظ خطة الحفظ PDF",
      review: "حفظ جدول المراجعة PDF",
      combined: "حفظ الخطة الشاملة PDF",
    };
    btn.textContent = labels[state.tab] || "حفظ الخطة PDF";
  }
  function renderTab() {
  ...
  // direction "reverse" ("الحفظ المعكوس"): السّور تُطوى تنازلياً — من آخر سورة في المقطع الأقرب لسورة
  // الناس رجوعاً نحو الفاتحة (مثلاً: فصلت ثم غافر) — بينما صفحات كل سورة على حدة تُحسب دائماً تصاعدياً
  // ابتداءً من أول صفحة فيها. حصة اليوم رقم ثابت (quota) لا يتغيّر بحسب حدود السور: إن كانت الحصة أكبر
  // مما تبقّى من السورة الحالية، يُستكمل أولاً ما تبقّى منها (تصاعدياً حتى آخر صفحة فيها)، ثم يُنتقل
  // مباشرة للسورة السابقة ويُكمل الباقي من حصة اليوم من أول صفحة فيها تصاعدياً — وهكذا قد يغطي اليوم
  // الواحد أكثر من نطاق صفحات منفصل دفعة واحدة (مثال: صفحة 482 من فصلت + من 467 إلى 470 من غافر).
  // هذه النطاقات المتعددة تُعاد في day.ranges (مصفوفة مرتبة من {from,to})؛ وتبقى fromPage/toPage على
  // كائن اليوم أيضاً (بداية أول نطاق ونهاية آخر نطاق) لأي كود قديم ما زال يتوقع زوجاً واحداً فقط.
  function buildSegmentedDailyAssignments(segments, totalDays, activeDaysList, startDateObj, direction) {
    ...
    function buildReverseBlocks() {
      ...
      orderedSegments.forEach((seg) => {
        let idx = surahIndexForPage(seg.to);
        while (true) {
          const sStart = surahStartPageOf(idx);
  ...
  // Returns the actual page segments the memorization plan (memoDays) has covered strictly before
  // calendar-day index `beforeIndex`, merged into the minimum number of {from,to} blocks. Unlike a
  // simple page COUNT, this captures exactly where the newly memorized pages sit in the book —
  // essential for "reverse" memorization (surahs walked from the end backward, pages inside each
  // surah ascending) where a single day can even cover two disjoint page ranges at once (finishing
  // one surah + starting the previous one) — so a plain min/max bounding box would wrongly include
  // pages that fall in between but were NOT actually memorized yet. Returns [] if nothing memorized.
  function getMemoSegmentsMemorizedBefore(memoDays, beforeIndex) {
    const raw = [];
    const limit = Math.min(beforeIndex, memoDays.length);
    for (let i = 0; i < limit; i++) {
      const d = memoDays[i];
  ...
  function buildGrowingCyclicAssignments(initialFrom, initialTo, cycleDays, totalDays, activeDaysList, startDateObj, memoDays) {
    ...
    while (offset < totalDays) {
      const chunkLen = Math.min(cycleDays, totalDays - offset);
      const chunkStart = addDays(startDateObj, offset);
      const newSegs = getMemoSegmentsMemorizedBefore(memoDays, offset);
      if (newSegs.length) segments = mergeReviewSegments([...segments, ...newSegs]);
  ...
  function getGrowingReviewFinalSegments(initialFrom, initialTo, memoDays) {
    let segments = [{ from: initialFrom, to: initialTo }];
    const finalSegs = getMemoSegmentsMemorizedBefore(memoDays, memoDays.length);
    if (finalSegs.length) segments = mergeReviewSegments([...segments, ...finalSegs]);
    return segments;
  }
  ...
  function buildMonthPageHTML(monthGroup, planTitle, planSubtitle, studentName) {
    ...
    for (let dnum = 1; dnum <= daysInMonth; dnum++) {
      ...
      } else if (a.isFilled || (a.fromPage == null && !(a.ranges && a.ranges.length))) {
        cells += `<div class="cal-cell cal-done">
          <span class="cal-date-num">${dnum}</span>
          <span class="cal-task">✓ تمّ الختم</span>
        </div>`;
      } else {
        // نستخدم الصيغة المختصرة داخل مربعات التقويم (مساحة ضيقة) حتى لا يطول النص عند تعدد نطاقات
        // الصفحات في يوم واحد (حالة الحفظ المعكوس)؛ الصيغة الكاملة تبقى للجدول النصي حيث المساحة أوسع.
        const label = formatDayRangesShort(a);
        const longCls = label.length > 11 ? " cal-task-long" : "";
        cells += `<div class="cal-cell">
          <span class="cal-date-num">${dnum}</span>
          <span class="cal-task${longCls}">${label}</span>
        </div>`;
      }
    }
  ...
  function buildCombinedMonthPageHTML(monthGroup, planTitle, planSubtitle, studentName) {
    ...
    for (let dnum = 1; dnum <= daysInMonth; dnum++) {
      ...
      if (!memoDay) {
        ...
        memoLabel = "راحة حفظ";
        memoIsRest = true;
      } else if (memoDay.isFilled || (memoDay.fromPage == null && !(memoDay.ranges && memoDay.ranges.length))) {
        memoLabel = "✓ تمّ الختم";
      } else {
        memoLabel = `حفظ: ${formatDayRangesShort(memoDay)}`;
      }
      ...
      // اليوم بدل أن يتمدد خارج حدوده المستديرة.
      const memoLongCls = memoLabel.length > 13 ? " long" : "";
      const reviewLongCls = reviewLabel.length > 13 ? " long" : "";
      const isRest = memoIsRest && reviewIsRest;
      cells += `<div class="cal-cell cal-cell-combined${isRest ? " cal-rest" : ""}">
        <span class="cal-date-num">${dnum}</span>
        <div class="cal-task-group">
          <span class="cal-task-mini memo${memoIsRest ? " is-rest" : ""}${memoLongCls}">${memoLabel}</span>
          <span class="cal-task-mini review${reviewIsRest ? " is-rest" : ""}${reviewLongCls}">${reviewLabel}</span>
        </div>
      </div>`;
    }
    // Pad out to exactly 6 full weeks (42 cells) so the grid always matches the
