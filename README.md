...
(function () {
  ...
  function computePace(m) {
    ...
    const activeDaysPerWeek = getActiveDaysCountPerWeek(m);
    const perDay = perWeek / activeDaysPerWeek; // pages per active memorization day
    return { totalWeeks, totalDays, totalYears, perDay, finishDate: addDays(startDate, totalDays), remaining };
  }
  ...
  function updateReviewResults() {
    ...
    if (r.cyclicMode) {
      ...
      const firstCycle = buildDailyAssignments(range.from, range.to, cycleDays, getEffectiveActiveDays(r), new Date());
  ...
  function updateCombinedResults() {
    ...
    const memoActiveDaysList = getEffectiveActiveDays(m);
    const memoDayLabel = (dayName) => isDayActive(dayName, memoActiveDaysList) ? `حفظ: ${memoPerDay.toFixed(1)}` : "راحة حفظ";
    ...
    if (r.cyclicMode) {
      ...
      weekGridHTML = firstCycle.map((d) => {
        ...
        <div class="day-card${d.isRest ? " day-rest" : ""}">
          ...
          <span class="day-pages">${memoDayLabel(dayName)}</span>
      ...
      weekGridHTML = schedule.weekPlan.map((d) => `
        <div class="day-card${d.isRest ? " day-rest" : ""}">
          <span class="day-name">${d.day}</span>
          <span class="day-pages">${memoDayLabel(d.day)}</span>
